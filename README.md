...
>
> **⚠️ 2026-09-24 重要实证修正（第 8 次抓包）**：用户执行「删除 App」（非卸载）重装后多次启动，开屏**立即出现**且 t=0 首请求就是开屏图（早于一切配置接口），请求的还是 09-16/17/18 的老 campaign 图；全包 78 会话搜索无任何响应携带 campaign URL、`GetAppStartImg` 全程 0 次调用 ⇒ **开屏 campaign 配置随 App 安装包自带**，删除重装无法消除。图片腿拦截完全生效（3/3 命中、全部换成 1×1 透明 GIF），开屏因此退化为「白屏 + 跳过倒计时 + 跳转按钮」的**空容器**——这是网络层能达到的最优状态，容器本身只能由 App 端移除，任何规则/重装都无效。此前「重装可根治」的说法据此作废。
>
