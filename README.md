...
| 22 | 豆瓣 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/douban_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Fdouban_splash_ad_block.plugin) | 过滤豆瓣 App 广告（开屏广告下发接口 / 广告位素材 / 电影页横幅），并拦截广告投放域、广告曝光计数与广告素材 CDN（含京东联盟投放的开屏素材）；纯本地规则，零远程脚本、不上报、不联网；需开启 MITM 并信任证书 |
...
| 24 | 天天基金 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/tiantianjijin_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Ftiantianjijin_splash_ad_block.plugin) | 拦截天天基金 App 开屏广告（断开屏下发接口 `GetAppStartImg` + 拦开屏图素材 `j5.dfcfw.com/WG/conf/<日期>/N<30位十六进制>.png`；放行 `/WG/appconf/` 以免误伤 App 配置图）；纯本地规则（接口拦截 + 图片素材拦截 + QUIC 阻断），零远程脚本、不上报、不联网；需开启 MITM 并信任证书 |
...
> **关于「天天基金」的开屏链路（2026-09-18 真机抓包定位后修正）**：开屏广告由数据接口 `appactive.1234567.com.cn/AppoperationApi/OperationService/GetAppStartImg` 下发，响应形如 `{"resultCode":0,...,"datas":[{"Title":"…活动…","ImgUrl":"…"}]}`。**同日实测 datas 的 5 条开屏图全部是活动推广图**（「震荡观市手记专题推广」「银行黄金满返活动」「盈利投资者数据专题」「everyclaw」「第五期黄金活动」），`ImgUrl` 全部指向 `j5.dfcfw.com/WG/conf/<8 位日期>/N<30 位十六进制>.png`。
>
> **⚠️ 本次定位到的真凶（前几版拦截失效的原因）**：线上真实文件名是 **`N` + 30 位十六进制**（整名 31 字符），而上一版正则写成 `N` + **32** 位 → **永远不命中，等于完全没拦**（这类"位数写错"不会报错、只能靠真机抓包发现）。本版已改为 30 位，并放宽为 `{28,40}`，避免 App 调整长度后再次静默失效。
>
> **⚠️ 目录踩坑（逐图下载核对内容后确认）**：`/WG/conf/<日期>/` 与 `/WG/appconf/<日期>/` 是两个不同目录，且 `/WG/conf/` 里**混着广告图与运营 UI 小图**——实测同一目录下既有 286KB 的开屏广告图（"淘金节…广告"）、58KB 的运营位广告图（"…广告"），也有 872B 的白色礼物盒 UI 图标。`/WG/appconf/` 实测为 App 配置类 UI 元素（3KB 的「详情」按钮）。两目录同命名规则、正则无法区分图的性质，因此本版**只拦 `/WG/conf/` 图片、放行 `/WG/appconf/`**，把误伤面压到最小；若你在首页看到某个运营小图变成灰块，那正是被这条规则一并拦掉的 `/WG/conf/` 内 UI 素材。
>
> **为什么「接口 + 图片」两条腿都要断**：只断接口不够——开屏图会被 App 预下载缓存，缓存命中时即使接口返回空数据 App 仍可能展示缓存图；社区主流方案（fmz200 / zirawell / junjiego2 / Qmxn 等）**全部同时拦截 j5 图片**，本插件据此采用两条腿同时断。
>
> **已知边界**：① 下发接口位于 App 核心 API 域 `appactive.1234567.com.cn`，若你的 App 版本对该域做了证书固定（certificate pinning），MITM 会失效、接口那条腿不生效，此时仅靠 j5 图片那条腿拦开屏；② 实测 `j5.dfcfw.com` 与 `appactive.1234567.com.cn` 均无 Alt-Svc（不走 QUIC），两域都支持明文 HTTP；③ 开屏图落地页域（`material.1234567.com.cn`）未拦截；④ 已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**。若图片那条腿也因故失效，纯本地已无更强手段，可改用 GKD（无障碍自动点「跳过」）绕过开屏。
>
