...
| 24 | 天天基金 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/tiantianjijin_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Ftiantianjijin_splash_ad_block.plugin) | 拦截天天基金 App 开屏广告（清空开屏下发接口 `GetAppStartImg` 数据 + 精确拦截当前开屏图 URL，不误伤首页图片；并从首页配置 `GetAppHome` 移除顶部横幅广告 `TopImgAd` 与弹窗广告 `HomePopupAd`）；纯本地规则（接口改写 + 图片精确拦截 + 首页模块剔除 + QUIC 阻断），零远程脚本、不上报、不联网；需开启 MITM 并信任证书；实测图片可全拦，开屏退化为白屏空容器（容器随 App 打包、删除重装也无法消除）；规则更新 2026-09-24 16:25 |
...
>
> **为什么接口和图片都要管**：只断接口不够 —— 开屏图会被 App **预下载缓存**，缓存命中时即使接口返回空数据 App 仍可能展示缓存图（社区 fmz200 的插件说明里就写着「某些 APP 要清除缓存或者重新安装拦截广告才会生效」）。
>
> **⚠️ 2026-09-24 重要实证修正（第 8 次抓包）**：用户执行「删除 App」（非卸载）重装后多次启动，开屏**立即出现**且 t=0 首请求就是开屏图（早于一切配置接口），请求的还是 09-16/17/18 的老 campaign 图；全包 78 会话搜索无任何响应携带 campaign URL、`GetAppStartImg` 全程 0 次调用 ⇒ **开屏 campaign 配置随 App 安装包自带**，删除重装无法消除。图片腿拦截完全生效（3/3 命中），开屏因此退化为「白屏 + 跳过倒计时 + 跳转按钮」的**空容器**——这是网络层能达到的最优状态，容器本身只能由 App 端移除，任何规则/重装都无效。此前「重装可根治」的说法据此作废。**2026-09-24 16:33 起黑名单动作由 `reject-img`（返回 1×1 透明图）改为 `reject`（直接断连）**：实验目标是让素材请求硬失败，赌 App 逻辑为「素材拉取失败 → 跳过本次开屏」，从而连白屏容器一起消除；最坏结果维持白屏（全新安装无缓存图可回退）。`tools/refresh_ttjj_splash.py` 已同步为 `reject`，自动刷新不会覆盖此改动。
>
> **已知边界**：① 下发接口位于 App 核心 API 域 `appactive.1234567.com.cn`，若该 App 版本对该域做了证书固定（certificate pinning），MITM 会失效、接口那条腿不生效，此时靠 `AUTO-SPLASH-URLS` 的精确 URL 兜住开屏；② 实测 `j5.dfcfw.com` 与 `appactive.1234567.com.cn` 均无 Alt-Svc（不走 QUIC）；③ 开屏图落地页域（`material.1234567.com.cn`）未拦截；④ MITM 含 `j5.dfcfw.com`（会解密该 CDN 的图片流量）；⑤ 已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**。参考：blackmatrix7 的规则集对天天基金**只拦接口、不碰 j5 图片**，本插件在此基础上补了精确 URL 兜底；⑥ 腿③ 所需，`appconfig2.1234567.com.cn`（首页配置接口域）也已加入 MITM——该域只承载 App 配置下发，不含用户数据。
>
