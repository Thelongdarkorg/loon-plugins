...
>
> **⚠️ 腿① 为什么不用 `reject-dict`（返回 `{}`），而用 `response-body-json-jq`**：本仓库在京东 / 闲鱼 / 途虎 三个插件上反复验证过同一规律 —— App 收到 `{}` 会判定为「接口异常」而**回退展示本地缓存的开屏**，表现为「隔几天偶发闪一次」；只有返回「成功但零条开屏」才能真正压住。本插件 2026-09-22 版据此把接口那条从 `reject-dict` 改成 jq 清空 `datas`。
>
> **⚠️ 为什么不能「按目录拦 `/WG/conf/`」（本仓库前几版踩过的坑）**：`/WG/conf/<日期>/` 是运营位共用的 CMS 素材目录，**同目录、同命名，混着广告图与首页运营位图片** —— 2026-09-18 实测同目录下既有 286KB 的开屏广告图、58KB 的运营位广告图，也有 872B 的白色礼物盒 UI 图标；2026-09-22 抓包再次实证：同一次冷启里 `/WG/conf/20260920/N7a248119….png`（299KB）是开屏广告图（文案「如何穿越震荡行情」，右下角带「广告」标），而 `/WG/conf/20260921/N6a80910f….png`（41KB）却是 `GetAppHome` 下发的**首页运营位 banner**（「热点 市场风格再平衡…一键配置」）。二者形状完全一致（8 位日期目录 + `N` + 30 位十六进制），正则**无法按性质区分**，按目录拦会把首页图片一起打死。已用 A/B 验证：conf 规则因正则位数写错而失效的那版，首页图标全部正常；规则一生效，首页图片就缺一半 → 证实 conf 里确实混着首页图片。`/WG/appconf/<日期>/` 则是 App 配置类 UI（3KB 的「详情」按钮，32 位纯 hex、无 `N` 前缀）。
>
...
点击上表「**一键导入**」链接，会经 Loon 官方中转页 `nsloon.com` 自动跳转并打开 Loon 完成安装。导入后插件列表中会显示该 App 的官方图标。
...
新插件请按表格格式在上方补一行。**名称只写 App 名**（如「京东」「小红书」，不要带「去广告」「开屏」等后缀，列宽才放得下），文件头部需包含：

```
#!name=App 名
#!desc=一句话说明（用途 + 依赖）
#!author=FFDNS[https://github.com/Thelongdarkorg/loon-plugins]
#!icon=https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/icons/<app>.jpg
```

`#!icon` 用于让 Loon 的插件列表显示对应 App 的图标，**图标统一托管在本仓库 `icons/` 目录**（192×192 JPEG，取自 App Store 官方图标），不要指向第三方 CDN 或别人的仓库。正文规则上方加 `#` 中文注释，说明拦的是什么。**不要使用 `script-path=` 指向远程脚本**。
