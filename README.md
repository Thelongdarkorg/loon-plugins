...
| 24 | 天天基金 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/tiantianjijin_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Ftiantianjijin_splash_ad_block.plugin) | 拦截天天基金 App 开屏广告（开屏下发接口 `GetAppStartImg` 返回空数据 + 开屏图素材 `j5.dfcfw.com/WG/conf/<日期>/` 按日期拒绝；放行 `/WG/appconf/` 以免误伤首页配置图）；纯本地规则（接口拦截 + 图片素材拦截 + QUIC 阻断），零远程脚本、不上报、不联网；需开启 MITM 并信任证书 |
...
> **关于「天天基金」的开屏链路（2026-09-18 修正为「接口 + 开屏图两条腿」）**：开屏广告由数据接口 `appactive.1234567.com.cn/AppoperationApi/OperationService/GetAppStartImg` 下发，响应形如 `{"resultCode":0,...,"datas":[{"Title":...,"ImgUrl":...}]}`，其中 `ImgUrl` 指向 `j5.dfcfw.com/WG/conf/<8 位日期>/N<32 位十六进制>.png`（实测 5 条开屏图全部如此）。**只断接口不够**——开屏图会被 App 预下载缓存，缓存命中时即使接口返回空数据 App 仍会展示缓存图；社区主流方案（fmz200 / zirawell / junjiego2 / Qmxn 等）**全部同时拦截 j5 图片**，本插件据此后改为「接口 + 开屏图」两条腿同时断。**关键收窄**：此前一版把 `/WG/conf/` 与 `/WG/appconf/` 合并成 `(?:app)?conf` 一起拦，导致首页运营位图标变成灰色占位图；本版**只拦 `/WG/conf/`、放行 `/WG/appconf/`**（后者更可能是「App 配置 / 首页运营位图」目录），并把开屏图特征收紧到实测形态 `/WG/conf/<8 位日期>/N<32 位十六进制>.<jpg|png>`，误伤概率更低。若你的设备上首页图标仍变灰，说明该 App 版本把首页图也放在 `/WG/conf/` 且与开屏图同命名（同目录同名无法用正则区分）——此时「拦开屏」与「显示首页图」二者不可兼得，属该 App 版本的固有限制（可改用 GKD 无障碍跳过开屏）。`j5.dfcfw.com` 无 Alt-Svc（不走 QUIC）；开屏图落地页域（`material.1234567.com.cn`）未拦截。已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**。
>
