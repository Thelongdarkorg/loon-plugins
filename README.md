...
> **关于「天天基金」的开屏链路（2026-09-18 修正为「只断接口」）**：开屏广告由数据接口 `appactive.1234567.com.cn/AppoperationApi/OperationService/GetAppStartImg` 下发，响应形如 `{"resultCode":0,...,"datas":[{"Title":...,"ImgUrl":...}]}`，其中 `ImgUrl` 指向 `j5.dfcfw.com/WG/conf/<8 位日期>/N<32 位十六进制>.png`（实测 5 条开屏图全部如此）。本插件**只在「下发接口」这一层断链** —— 把响应换成空对象 `{}`，App 解析成功但拿不到任何开屏素材，直接走自带启动页。**不再拦截 `j5.dfcfw.com` 的图片素材，也不再对该域开 MITM**：因为 `j5.dfcfw.com/WG/conf/<日期>/` 是天天基金「运营位」共用的 CMS 素材目录，首页的 活期宝 / 稳健理财 / 进阶理财 / 高端臻享 / 投顾管家 以及首页应用九宫格（定投专区 / 指数宝 / 新友基金 / 股票交易 / 排行 / 净值 / 基金主题…）图标同样从该目录下发，按「目录 + 图片扩展名」做图片级拦截会把主页图标一起打成灰色占位图（用户实测复现）。开屏图落地页域（`material.1234567.com.cn`）未拦截。已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**，二者互补不冲突。
>
