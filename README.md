...
| 24 | 天天基金 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/tiantianjijin_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Ftiantianjijin_splash_ad_block.plugin) | 拦截天天基金 App 开屏广告（开屏下发接口 `GetAppStartImg` + `j5.dfcfw.com` 按日期目录分发的开屏图素材）；纯本地规则（接口拦截 + 素材拦截 + QUIC 阻断），零远程脚本、不上报、不联网；需开启 MITM 并信任证书 |
...
> **关于「天天基金」的开屏链路**：开屏广告靠两条腿下发 —— ① 数据接口 `appactive.1234567.com.cn/AppoperationApi/OperationService/GetAppStartImg` 返回开屏数据（含开屏图 `ImgUrl` 与落地页）；② 图片素材托管在 `j5.dfcfw.com`。2026-09-18 实测该接口响应里 5 条开屏图**全部**指向 `j5.dfcfw.com/WG/conf/<8 位日期>/N<32 位十六进制>.png`，图片实测可正常下载（`image/png`）。本插件把「下发接口返回空对象」与「图片素材返回空体 200」同时断掉，开屏无内容可展示。上游同名插件（fmz200 奶思）把 `WG/conf` 与 `WG/appconf` 写成两条规则，本仓库合并为一条，并把日期目录放宽到任意 8 位数字（上游写死 `202[0-9]{5}`，仅覆盖 2020–2029 年）、扩展名大小写不敏感。开屏图落地页域（`material.1234567.com.cn`）**未拦截** —— 开屏已无内容，且该域同时承载非广告页面。已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**，二者互补不冲突。
