...
| 24 | 天天基金 | [Raw 链接](https://raw.githubusercontent.com/Thelongdarkorg/loon-plugins/main/tiantianjijin_splash_ad_block.plugin) ｜ [一键导入](https://www.nsloon.com/openloon/import?plugin=https%3A%2F%2Fraw.githubusercontent.com%2FThelongdarkorg%2Floon-plugins%2Fmain%2Ftiantianjijin_splash_ad_block.plugin) | 拦截天天基金 App 开屏广告（开屏下发接口 `GetAppStartImg` 返回空数据，不开屏图）；纯本地规则（接口拦截 + QUIC 阻断），零远程脚本、不上报、不联网；需开启 MITM 并信任证书 |
...
> **关于「天天基金」的开屏链路（2026-09-18 修正为「连接层 URL-REGEX 拒绝」）**：开屏广告由数据接口 `appactive.1234567.com.cn/AppoperationApi/OperationService/GetAppStartImg` 下发，响应形如 `{"resultCode":0,...,"datas":[{"Title":...,"ImgUrl":...}]}`，其中 `ImgUrl` 指向 `j5.dfcfw.com/WG/conf/<8 位日期>/N<32 位十六进制>.png`（实测 5 条开屏图全部如此）。本插件用 **Rule 层 `URL-REGEX` 在连接层直接拒绝该 URL**（精确只匹配 `GetAppStartImg` 这一条路径，不影响同域的行情/交易/自选/首页配置），App 请求被掐掉后拿不到开屏数据、直接走自带启动页。**不再拦截 `j5.dfcfw.com` 的图片素材，也不再对该域开 MITM**：因为 `j5.dfcfw.com/WG/conf/<日期>/` 是天天基金「运营位」共用的 CMS 素材目录，首页的 活期宝 / 稳健理财 / 进阶理财 / 高端臻享 / 投顾管家 以及首页应用九宫格（定投专区 / 指数宝 / 新友基金 / 股票交易 / 排行 / 净值 / 基金主题…）图标同样从该目录下发，按「目录 + 图片扩展名」做图片级拦截会把主页图标一起打成灰色占位图（用户实测复现）。**⚠️ 证书固定限制**：`appactive.1234567.com.cn` 是东方财富核心 API 域，被 App 做了证书固定（certificate pinning）——Loon 的 MITM 自签证书通不过校验，导致基于路径的响应体改写（`reject-dict`）与按路径匹配全部静默失效；若改用「域名级 REJECT 整个 appactive 域」又会连坐行情/交易/自选/首页配置（整个 App 断网）。因此当你的 App 版本已固定该域时，**纯本地方案无法「既拦开屏又不破坏功能」**——本插件退化为「只保证首页 UI 正常」，开屏广告可能照常显示，属当前版本 App 的天花板。开屏图落地页域（`material.1234567.com.cn`）未拦截。已核对与仓库「东财」插件（`emdcadvertise.eastmoney.com` / `swdlcdn.eastmoney.com`）**零重叠**，二者互补不冲突。
>
